...
library tlsnutils{
  ...
  */
  function getConversationPart(bytes memory proof, bytes1 conversation_part) private pure returns(bytes) {
    ...
    for(uint16 i = 0; i < uint16(proof[6])+256*uint16(proof[7]); i++){
      ...
      if(generator_originated == conversation_part){
          bytesutils.slice memory chunk = proof.toSlice(readPos).truncate(tmplen);
  ...
  */
  function getRequest(bytes memory proof) internal pure returns(bytes) {
   ...
   * @return The response as a bytestring.
  */
  function getResponse(bytes memory proof) internal pure returns(bytes) {
  ...
  */
  function getHTTPBody(bytes memory proof) internal pure returns(bytes) {
  ...
  */
  function getHost(bytes memory proof) internal pure returns(bytes) {
  ...
  */
  function getHTTPRequestURL(bytes memory proof) internal pure returns(bytes) {
  ...
  */
   ...
   * @return True iff valid.
  */
	function verifyProof(bytes memory proof, uint256 qx, uint256 qy) public pure returns(bool) {
  ...
  */
  function parseProof(bytes memory proof) public pure returns(uint256 sig_r, uint256 sig_s, bytes32 hashchain) {
    ...
    readPos += uint16(proof[26])+256*uint16(proof[27]);
	  ...
	  for(uint16 i = 0; i < uint16(proof[6])+256*uint16(proof[7]); i++){
			...
			// Root hash
			bytes32 hash = sha256(abi.encodePacked(saltsecret,chunk,uint8(0),len_record,generator_originated));
			// Hash chain
			if(i == 0){
				hashchain = sha256(abi.encodePacked(uint8(1),hash));
			}else{
				hashchain = sha256(abi.encodePacked(uint8(1),hashchain,hash));
			}
		...
		hashchain = sha256(abi.encodePacked(hashchain, times, test, uint32(0x04000000)));
  }
}
